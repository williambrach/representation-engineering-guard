...
def honesty_function_dataset(
    ...
    combined_data = [
        [honest, untruthful]
        for honest, untruthful in zip(honest_statements, untruthful_statements, strict=False)
    ]
    ...
    reshaped_data = np.array(
        [
            ...
            for honest, untruthful in zip(
                honest_statements[:-1], untruthful_statements[1:], strict=False
            )
        ]
    ).flatten()
...
def plot_detection_results(
    ...
    for rep, s_style, n_style in zip(selected_concepts, selection_style, norm_style, strict=False):
        ...
        for word, score in zip(words[5:], rep_scores[5:], strict=False):
