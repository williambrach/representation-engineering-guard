...
def literary_openings_dataset(data_dir, ntrain=16, seed=0):
    ...
    data = [
        [s.replace("...", ""), u.replace("...", "")]
        for s, u in zip(seen_docs, unseen_docs, strict=False)
    ]
...
def quotes_dataset(data_dir, ntrain=16, seed=0):
    ...
    data = [[s, u] for s, u in zip(seen_quotes, unseen_quotes, strict=False)]
    train_data = data[:ntrain]
    test_data = data
...
def eval_completions(outputs, targets):
    ...
    em = np.mean([t in o for t, o in zip(targets, outputs, strict=False)])
