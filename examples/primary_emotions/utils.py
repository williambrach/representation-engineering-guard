...
def primary_emotions_concept_dataset(data_dir, user_tag="", assistant_tag="", seed=0):
    ...
    for emotion in emotions:
        c_e, o_e = raw_data[emotion], np.concatenate(
            [v for k, v in raw_data.items() if k != emotion]
        )
        random.shuffle(o_e)

        data = [[c, o] for c, o in zip(c_e, o_e, strict=False)]
        train_labels = []
        for d in data:
            true_s = d[0]
            random.shuffle(d)
        ...
        data_ = np.concatenate([[c, o] for c, o in zip(c_e, o_e, strict=False)]).tolist()
...
def primary_emotions_function_dataset(data_dir, user_tag="", assistant_tag="", seed=0):
    ...
    train_template_str = (
        "{user_tag} Act as if you are extremely {emo}. {assistant_tag} {scenario}"
    )
    emotions = ["happiness", "sadness", "anger", "fear", "disgust", "surprise"]
    with open(os.path.join(data_dir, "all_truncated_outputs.json")) as file:
    ...
    formatted_data = {}
    for emotion, emotion_adj, emotion_adj_ant in zip(
        emotions, emotions_adj, emotions_adj_ant, strict=False
    ):
        emotion_train_data_tmp = [
            [
                train_template_str.format(
                    emo=np.random.choice(emotion_adj),
                    scenario=s,
                    user_tag=user_tag,
                    assistant_tag=assistant_tag,
                ),
                train_template_str.format(
                    emo=np.random.choice(emotion_adj_ant),
                    scenario=s,
                    user_tag=user_tag,
                    assistant_tag=assistant_tag,
                ),
