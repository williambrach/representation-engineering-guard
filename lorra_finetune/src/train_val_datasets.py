...
def get_truncated_outputs(
    ...
    orig_s, pos_s, neg_s = [], [], []
    for s, p in zip(all_outputs, prefixes, strict=False):
        orig_s.append(
        ...
        neg_s.append(
            neg_template.format(
                ...
                assistant_tag=assistant_tag,
                instruction=p,
                type=control_template.format(type=neg_type),
                response=s,
            )
        )
...
class AlpacaSupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> dict[str, torch.Tensor]:
        ...
        combined_input_ids = torch.cat(
            [tokenized_inputs["input_ids"], response_tokenized_inputs["input_ids"]],
            dim=1,
        )
        combined_attention_mask = torch.cat(
            [
                tokenized_inputs["attention_mask"],
                response_tokenized_inputs["attention_mask"],
            ],
            dim=1,
        )
        return dict(
...
def prepare_decoder_only_inputs(prompts, targets, tokenizer, device):
    ...
    inputs = {
        k: torch.cat([prompt_inputs[k], target_inputs[k]], dim=1) for k in prompt_inputs
    }
...
def get_logprobs_accuracy(model, tokenizer, questions, answers, labels, bsz):
    ...
    for i in range(len(questions) // bsz + 1):
        ...
        inputs, masks = prepare_decoder_only_inputs(
            q_batch, a_batch, tokenizer, model.device
        )
        with torch.no_grad():
