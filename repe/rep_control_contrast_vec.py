...
def contrast_greedy_search(
    ...
    **model_kwargs,
) -> GreedySearchDecoderOnlyOutput | torch.LongTensor:
    ...
    assert not compute_contrast or not model_kwargs.get(
        "use_cache", False
    ), "Contrast Greedy Search not yet support generate with use_cache, please set model.generate(**kwargs, use_cache=False)"
...
def forward_contrast_vector(
    ...
    if (isinstance(self, LlamaModel) and self._use_flash_attention_2) or (
        isinstance(self, MistralModel)
        ...
        (isinstance(self, LlamaModel) and self._use_sdpa)
        or (
            isinstance(self, MistralModel)
    ...
    for layer_id, decoder_layer in enumerate(self.layers):
        ...
        if compute_contrast:
            ...
            if layer_id in control_layer_ids:
                activations = alpha * (
                    hidden_states_p[:, contrast_tokens:]
                    - hidden_states_n[:, contrast_tokens:]
                )
    ...
    def forward(
        ...
        **repe_kwargs,
        ...
        if self.config.pretraining_tp > 1:
            lm_head_slices = self.lm_head.weight.contrast_tokens(
