...
def main(
        ...
        for layer in hidden_layers:
            ...
            unflattened_H_tests = [
                list(
                    islice(
                        H_test,
                        sum(len(c) for c in labels[:i]),
                        sum(len(c) for c in labels[: i + 1]),
                    )
                )
            ...
            cors = np.mean(
                [
                    labels[i].index(1) == eval_func(H)
                    for i, H in enumerate(unflattened_H_tests)
                ]
            )
    ...
    if dataset["val"]:
        ...
        print(
            f"Test Acc for chosen layer: {best_layer} - {results['test'][best_layer_idx]}"
        )
    else:
