...
def arc_dataset(config, ntrain=25, seed=1):
    ...
    def format_samples(df, idx):
        ...
        prompts.append(
            template_str.format(question=question, answer=clean_answer_s(true_answer_s))
        )

        for false_answer_s, i in [
            (a, i) for i, a in enumerate(prompt_choices_text) if a != true_answer_s
        ]:
            prompts.append(
                template_str.format(
                    question=question, answer=clean_answer_s(false_answer_s)
                )
            )
    ...
    train_df = (
        dataset["train"].filter(_keep_4_options_row).shuffle(seed=seed).to_pandas()
    )
