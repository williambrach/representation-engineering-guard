...
def _collate_data(set):
    ...
    for item in load_dataset(path="race", name="high")[set]:
...
def race_dataset(ntrain=3, seed=0):
    ...
    def format_samples(df, idx):
        ...
        true_answer_s = template_str.format(
            question=question, answer=answer_s, context=context
        )
        prompts.append(true_answer_s)
        for o in options:
            if o == answer_s:
                continue
            false_answer_s = template_str.format(
                question=question, answer=o, context=context
            )
    ...
    val_data, val_labels = samples(val_df)[
        :200
    ]  # use subset because 70B eval is expensive
